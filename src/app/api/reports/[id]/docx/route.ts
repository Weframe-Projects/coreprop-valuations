// Vercel serverless: allow up to 60s for DOCX generation
export const maxDuration = 60;

import { NextResponse, type NextRequest } from 'next/server';
import { createClient } from '@/lib/supabase/server';
import { generateDocx, type GenerateDocxInput } from '@/lib/docx-generator';
...
// POST /api/reports/[id]/docx
// Generates a Word document (.docx) from the stored report data and returns the binary.
export async function POST(
  ...
  if (!row.valuation_figure) {
    return NextResponse.json(
      { error: 'Please enter a valuation figure before generating the document.' },
      { status: 400 }
    );
  }
  ...
  // Build docx input
  const docxInput: GenerateDocxInput = {
    ...
    firmSettings: settings ? {
      ...
      address: 'First Floor, 4 Pentonville Road, London, N1 9HF',
    } : undefined,
  };

  try {
    const docxBuffer = await generateDocx(docxInput);

    const fileName = `CoreProp-${row.postcode.replace(/\s/g, '')}-${row.reference_number || id.slice(0, 8)}.docx`;
    ...
    if (driveFolderId && user) {
      ...
      }).catch((err) => console.error('[docx] Drive upload failed (non-blocking):', err));
    }
    ...
    return NextResponse.json(
      { error: 'Word document generation failed. Please try again.' },
      { status: 500 }
    );
  }
}
